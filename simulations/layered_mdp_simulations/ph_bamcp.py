...
class EarlyStoppingBAMCPSolver(BAMCPSolver):
    ...
    def is_stalled(self, var_history, window=20, tol=0.05):
        """
        ...
        # REMEMBER TO CHANGE THE WINDOW LENGTH 
        #print("predicted change: ", predicted_change)
        return abs(predicted_change) < tol
    
    def check_stopping_criterion(self, acc_fisher_information, window=20, tol=0.05):
        """
        Trend-based stopping check. NOT derived from the theorems. Window
        and tol are heuristic tuning knobs, that we calibrate 
        ...
        stopped = self.is_stalled(trace_history, window, tol)
    ...
    def run_bamcp(self, s0=None, total_reward=0, total_steps=0, is_defer_vec=None, is_advice_vec=None, is_auto_vec=None, belief_vec=None, belief_stats=None, rewards=None, cum_rewards=None, max_steps=np.inf, window=20, tol=0.05):
        state = s0 if s0 is not None else self.bamdp.s0 
        history = History(items=(state,))
        total_reward = total_reward 
        total_steps = total_steps
        ...
        while not self.bamdp.is_goal(state) and not self.bamdp.is_terminal(state) and total_steps < max_steps and not stopped:
           ...
           # if issued_action[0] == 0:
            ...
            if operator in self.bamdp.boltzmann_operators:
                ...
                stopped, trigger_param, final_var_beta, final_var_alpha = self.check_stopping_criterion(acc_fisher_information=fisher_accumulator[opidx], window=window, tol=tol)
    ...
    def run(self, s0=None, total_reward=0, total_steps=0, is_defer_vec=None, is_advice_vec=None, is_auto_vec=None, belief_vec=None, belief_stats=None, rewards=None, cum_rewards=None, max_steps=np.inf, window=20, tol=0.05):
